            ...
            // whether or not we choose to write new syntax or old syntax, we always have to REFER to our state object with the "this" keyword*/}
					...
					this.state.products.map((product, index) => {
						return (
							...
							// each element inside our array is its own object
              // our paramenter product is referencing to each element in the array while we map over it
                // WARNING - each child in a list should have a unique ID
                // the most typicla way to assign a unique ID to a list item is by using the "i" iterator
                // we need to add an extra parameter inside our arrow function inside our map function
                <ProductList key={index} product= {product}/>

                // instead of the old way where we use li to go through all of the products and price like this: 
                // <li>
